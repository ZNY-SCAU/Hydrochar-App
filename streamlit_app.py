    ...
    with cols_main[i]:
        ...
        if not valid_feats:
            ...
            for feat in valid_feats:
                ...
                sub_c2.caption(f"{stat['min']:.3f}~{stat['max']:.3f}")
                ...
                # 🚀🚀🚀【修复核心】结果显示逻辑独立出来！🚀🚀🚀
                # 无论是否锁定(should_lock状态)，只要有结果，就在这里显示！
                if feat in activation_feats and is_activation_locked():
                    ...
                    st.markdown(f"<div style='text-align:right; color:#27AE60; font-weight:bold'>✅ {res_v:.4f}</div>", unsafe_allow_html=True)
